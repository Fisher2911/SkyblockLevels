...
public class BlockBreakData {
    ...
    public static final byte MAX_DAMAGE = 9;
    ...
    public BlockBreakData(Player player, ItemStack breakingWith, int totalTicks, Consumer<WorldPosition> onBreak) {
        this.player = player;
    ...
    public boolean isBroken() {
        return this.calculateDamage() == MAX_DAMAGE;
    }
    ...
    public byte calculateDamage() {
        ...
        return (byte) Math.min(damage, MAX_DAMAGE);
    }

    public void send(WorldPosition position) {
        final byte calculatedDamage = this.calculateDamage();
        PacketHelper.sendBlockBreakAnimation(this.player, position.toLocation(), calculatedDamage);
    }
